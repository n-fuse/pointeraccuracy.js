...
var Pointeraccuracy = {
		...
		classify: function() {
			...
			if (thiz.pointerModeNative !== null) { // If pointer MQ is supported, just use its result
				...
				if ('ontouchstart' in window) { // If the browser uses touch events, use that as the result
					mode = 'coarse';
				} else if (thiz.hasDoubleDevicePixelRatio()) { // Screens with DPR = 2 can these days only be found on small-medium screen touch devices 
					mode = 'coarse';
				} else {
